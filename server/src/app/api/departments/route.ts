...
const createDepartmentSchema = z.object({
  ...
  code: z.string().min(2, 'Department code must be at least 2 characters long').toUpperCase(),
});
...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  if (existing) {
    throw new ValidationError('A department with this name or code already exists');
  }
